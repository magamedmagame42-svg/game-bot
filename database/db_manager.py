...
def add_user(user_id, name):
    """Добавляет нового игрока в базу, если его нет"""
    conn = sqlite3.connect('users.db')
    cursor = conn.cursor()
    cursor.execute('INSERT OR IGNORE INTO users VALUES (?, ?, ?)', (user_id, name, 100)) # Даем 100 монет при старте
    conn.commit()
    conn.close()
...
def get_coins(user_id):
    """Возвращает текущий баланс монет игрока"""
    conn = sqlite3.connect('users.db')
    cursor = conn.cursor()
